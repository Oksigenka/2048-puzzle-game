...
function keyMove(ev) {
    ...
    if (game.isMoveMade(previousField)) game.generateCell();
    game.getStatus();
    game.getScore();
    vizualized();
...
function vizualized() {
    for(let i = 0; i < 4; i++){
        for(let j = 0; j < 4; j++)if (game.field[i][j] === 0) table.rows[i].cells[j].textContent = "";
        else table.rows[i].cells[j].textContent = game.field[i][j];
    }
    colorCells();
    if (!game.canMove()) messageLose.style.display = "block";
    if (game.winerGame()) messageWin.style.display = "block";
}
function colorCells() {
